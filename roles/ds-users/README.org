...
user_data:
  user1:
    ssh_key: "ssh-rsa ..."
    full_name: "User 1"
    password: "*"       # Default disabled password
    groups:
      - ansible
...
| =sudo=      | dict   | -       | Ignored, used by =ds-sudo=.                   |

For *real password hashes*, store them in an Ansible Vault or SOPS in the =vault_data= hash.
...
* Integration
This role is designed to integrate with =ds-sudo= and =ds-ssh=, forming the base layer of DeadSwitch system provisioning.
